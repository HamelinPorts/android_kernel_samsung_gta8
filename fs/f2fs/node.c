...
static void f2fs_print_lower_node(struct f2fs_sb_info *sbi, struct page *page)
{
	...
	/*
	 * Try read upper (decrypted block) block one more time
	 */
	ClearPageUptodate(page);
	...
	lock_page(page);
	if (PageUptodate(page)) {
		f2fs_warn(sbi, "Print upper nid block");
		print_block_data(sbi->sb, page->index, page_address(page), 0, F2FS_BLKSIZE);
	}
}
...
int f2fs_flush_nat_entries(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
	...
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct f2fs_journal *journal = curseg->journal;
	struct nat_entry_set *setvec[SETVEC_SIZE] = {0};
...
void f2fs_destroy_node_manager(struct f2fs_sb_info *sbi)
{
	...
	struct nat_entry *natvec[NATVEC_SIZE] = {0};
	struct nat_entry_set *setvec[SETVEC_SIZE] = {0};
	nid_t nid = 0;
	unsigned int found;
