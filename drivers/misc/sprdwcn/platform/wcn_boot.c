...
static int marlin_parse_dt(struct platform_device *pdev)
{
	...
	marlin_dev->rfcustom = devm_regulator_get(&pdev->dev, "rfcustom");
	if (IS_ERR(marlin_dev->rfcustom)) {
		if (PTR_ERR(marlin_dev->rfcustom) == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		pr_err("Get regulator of rfcustom error!\n");
	}
...
/* Tab code for SR-AX6300-01-193 by qiaodan at 20210809 start */
static void rfcustom_power_enable(bool enable)
{
	int ret = -1;

	if (marlin_dev->rfcustom) {
		pr_info("rfcustom power%d\n", enable);
		usleep_range(4000, 5000);
		if (enable) {
			if (regulator_is_enabled(marlin_dev->rfcustom))
				return;

			regulator_set_voltage(marlin_dev->rfcustom,
					      2800000, 2800000);
			ret = regulator_enable(marlin_dev->rfcustom);
			if (ret)
				pr_err("fail to enable rfcustom power\n");
		} else {
			if (regulator_is_enabled(marlin_dev->rfcustom)) {
				ret = regulator_disable(marlin_dev->rfcustom);
				if (ret)
					pr_err("fail to disable rfcustom power\n");
			}
		}
	}
}
