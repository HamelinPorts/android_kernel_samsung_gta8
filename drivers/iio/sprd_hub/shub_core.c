...
/*Tab A8 code for SR-AX6300-01-65 by xiongxiaoliang at 2021/08/25 end*/
...
static int send_lcd_info(struct shub_data *sensor)
{
	...
	err = shub_send_command(sensor, SENSOR_LIGHT, AP_SEND_DATA_TO_DYNAMIC_SUBTYPE,
				lcd_info, CALIBRATION_DATA_LENGTH);
	if (err < 0)
		dev_err(&sensor->sensor_pdev->dev, "Send Lcd info Fail\n");
...
static void shub_download_calibration_data_work(struct work_struct *work)
{
	...
	if (sensor->mcu_mode == SHUB_NORMAL) {
		...
		send_lcd_info(sensor);
		first_light_calibration_data_send(sensor);
		second_light_calibration_data_send(sensor);
	}
}
/*Tab A8 code for SR-AX6300-01-65 by xiongxiaoliang at 2021/08/25 end*/
...
/* HS03 code for SR-SL6215-01-162 by shibinbin at 2021/08/11 start */
/*Tab A8 code for SR-AX6300-01-72 by mayuhang at 2021/8/20 start*/
static ssize_t acc_calibration_data_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
    int err;
    struct file *pfile;
    char file_path[CALIB_PATH_MAX_LENG];
    char *acc_cali_data = NULL;
    int acc_calidata[3] = {10000 , 10000, 10000};

    if ((strlen(CALIBRATION_NODE) + strlen(calibration_filename[1])) >
        (sizeof(file_path) - 1)) {
        pr_err("calibration node path is oversize.\n");
        return -EINVAL;
    }
    snprintf(file_path, sizeof(file_path), "%s%s",
        CALIBRATION_NODE, calibration_filename[1]);
    ...
    acc_cali_data = kmalloc(CALIBRATION_DATA_LENGTH, GFP_KERNEL);
    if (acc_cali_data == NULL) {
        filp_close(pfile, NULL);
        return -ENOMEM;
    }

    err = kernel_read(pfile, acc_cali_data,
    ...
    memcpy(acc_calidata, acc_cali_data, sizeof(acc_calidata));

    kfree(acc_cali_data);
    filp_close(pfile, NULL);

    return sprintf(buf, "%d,%d,%d\n", acc_calidata[0],acc_calidata[1],acc_calidata[2]);
}
static DEVICE_ATTR_RO(acc_calibration_data);
/*Tab A8 code for SR-AX6300-01-72 by mayuhang at 2021/8/20 end*/
/* HS03 code for SR-SL6215-01-162 by shibinbin at 2021/08/11 end */
...
/*Tab A8 code for AX6300DEV-48 by xiongxiaoliang at 2021/08/19 start*/
static ssize_t gyro_calibration_data_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int err;
    struct file *pfile;
    char file_path[CALIB_PATH_MAX_LENG];
    char *gyro_cali_data = NULL;
    int gyro_calidata[3] = {10000 , 10000, 10000};

    if ((strlen(CALIBRATION_NODE) + strlen(calibration_filename[4])) >
        (sizeof(file_path) - 1)) {
        pr_err("calibration node path is oversize.\n");
        return -EINVAL;
    }
    snprintf(file_path, sizeof(file_path), "%s%s",
        CALIBRATION_NODE, calibration_filename[4]);
    ...
    gyro_cali_data = kmalloc(CALIBRATION_DATA_LENGTH, GFP_KERNEL);
    if (gyro_cali_data == NULL) {
        filp_close(pfile, NULL);
        return -ENOMEM;
    }

    err = kernel_read(pfile, gyro_cali_data,
                  CALIBRATION_DATA_LENGTH,
                  &pfile->f_pos);
    if (err < 0) {
        pr_err("Error: file read failed\n");
        kfree(gyro_cali_data);
        filp_close(pfile, NULL);
        return err;
    }

    memcpy(gyro_calidata, gyro_cali_data, sizeof(gyro_calidata));

    kfree(gyro_cali_data);
    filp_close(pfile, NULL);

    return sprintf(buf, "%d,%d,%d\n", gyro_calidata[0], gyro_calidata[1], gyro_calidata[2]);
}
static DEVICE_ATTR_RO(gyro_calibration_data);
/*Tab A8 code for AX6300DEV-48 by xiongxiaoliang at 2021/08/19 end*/
...
static void shub_save_calibration_data(struct work_struct *work)
{
	...
	/*Tab A8 code for AX6300DEV-81 by xiongxiaoliang at 2021/08/25 start*/
	pfile = filp_open(file_path, O_WRONLY | O_CREAT | O_TRUNC, 0666);
	/*Tab A8 code for AX6300DEV-81 by xiongxiaoliang at 2021/08/25 end*/
	if (IS_ERR(pfile)) {
		err = PTR_ERR(pfile);
		dev_err(&sensor->sensor_pdev->dev,
			"open file %s error=%d\n", file_path, err);
...
/* HS03 code for SR-SL6215-01-162 by shibinbin at 2021/08/27 start */
static ssize_t light_calibration_data_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
    int err;
    struct file *pfile;
    char file_path[CALIB_PATH_MAX_LENG];
    char *light_cali_data = NULL;
    int light_calidata = 10000;
    ...
    light_cali_data = kmalloc(CALIBRATION_DATA_LENGTH, GFP_KERNEL);
    if (light_cali_data == NULL) {
        filp_close(pfile, NULL);
        return -ENOMEM;
    }

    err = kernel_read(pfile, light_cali_data,
                  CALIBRATION_DATA_LENGTH,
                  &pfile->f_pos);
    if (err < 0) {
        pr_err("Error: file read failed\n");
        kfree(light_cali_data);
        filp_close(pfile, NULL);
        return err;
    }

    memcpy(&light_calidata, light_cali_data, sizeof(light_calidata));

    kfree(light_cali_data);
    filp_close(pfile, NULL);
	return sprintf(buf, "%d\n", light_calidata);
}
static DEVICE_ATTR_RO(light_calibration_data);
/* HS03 code for SR-SL6215-01-162 by shibinbin at 2021/08/27 end */
...
/*Tab A8 code for SR-AX6300-01-65 by xiongxiaoliang at 2021/08/25 start*/
/* Dual light sensor type */
#define SENSOR_TYPE_FIRST_LIGHT 123
#define SENSOR_TYPE_SECOND_LIGHT 124
#define CALI_VAL_SAVE_ADDR_OFFSET 4
#define CALI_COEF_DEFAULT_VAL 1000

/*Create read calibration factor node*/
static ssize_t first_light_calibration_data_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int err;
    struct file *pfile;
    char file_path[CALIB_PATH_MAX_LENG];
    char *firstlight_cali_data = NULL;
    int firstlight_calidata = 0;

    if ((strlen(CALIBRATION_NODE) + strlen(calibration_filename[10])) >
    (sizeof(file_path) - 1)) {
        pr_err("calibration node path is oversize.\n");
        return -EINVAL;
    }
    snprintf(file_path, sizeof(file_path), "%s%s",
        CALIBRATION_NODE, calibration_filename[10]);

    pfile = filp_open(file_path, O_RDONLY, 0);
    if (IS_ERR(pfile)) {
        err = PTR_ERR(pfile);
        pr_err("open file %s ret=%d\n", file_path, err);
        return sprintf(buf, "%d\n", CALI_COEF_DEFAULT_VAL);
    }

    firstlight_cali_data = kmalloc(CALIBRATION_DATA_LENGTH, GFP_KERNEL);
    if (firstlight_cali_data == NULL) {
        filp_close(pfile, NULL);
        return -ENOMEM;
    }

    err = kernel_read(pfile, firstlight_cali_data,
                  CALIBRATION_DATA_LENGTH,
                  &pfile->f_pos);
    if (err < 0) {
        pr_err("Error: file read failed\n");
        kfree(firstlight_cali_data);
        filp_close(pfile, NULL);
        return err;
    }

    pr_err("first_light_calibration_data_show\n");
    memcpy(&firstlight_calidata, firstlight_cali_data + CALI_VAL_SAVE_ADDR_OFFSET, sizeof(firstlight_calidata));
...
static DEVICE_ATTR_RO(first_light_calibration_data);

static ssize_t second_light_calibration_data_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int err;
    struct file *pfile;
    char file_path[CALIB_PATH_MAX_LENG];
    char *secondlight_cali_data = NULL;
    int secondlight_calidata = 0;

    if ((strlen(CALIBRATION_NODE) + strlen(calibration_filename[11])) >
    (sizeof(file_path) - 1)) {
        pr_err("calibration node path is oversize.\n");
        return -EINVAL;
    }
    snprintf(file_path, sizeof(file_path), "%s%s",
        CALIBRATION_NODE, calibration_filename[11]);
    ...
    secondlight_cali_data = kmalloc(CALIBRATION_DATA_LENGTH, GFP_KERNEL);
    if (secondlight_cali_data == NULL) {
        filp_close(pfile, NULL);
        return -ENOMEM;
    }

    err = kernel_read(pfile, secondlight_cali_data,
                  CALIBRATION_DATA_LENGTH,
                  &pfile->f_pos);
    if (err < 0) {
        pr_err("Error: file read failed\n");
        kfree(secondlight_cali_data);
        filp_close(pfile, NULL);
        return err;
    }
    pr_err("second_light_calibration_data_show\n");

    memcpy(&secondlight_calidata, secondlight_cali_data + CALI_VAL_SAVE_ADDR_OFFSET, sizeof(secondlight_calidata));

    kfree(secondlight_cali_data);
    filp_close(pfile, NULL);

    return sprintf(buf, "%d\n", secondlight_calidata);
}
static DEVICE_ATTR_RO(second_light_calibration_data);

static ssize_t first_light_calibration_status_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int err, cali_data_coef;
    int status = CALIB_STATUS_NON;
    ...
    memcpy(&cali_data_coef, raw_cali_data + CALI_VAL_SAVE_ADDR_OFFSET, sizeof(cali_data_coef));
    pr_info("first_light cali_data_coef = %d\n", cali_data_coef);
    if (cali_data_coef < 0)
        status = CALIB_STATUS_FAIL;
    else
        status = CALIB_STATUS_PASS;
...
static DEVICE_ATTR_RO(first_light_calibration_status);

static ssize_t second_light_calibration_status_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int err, cali_data_coef;
    int status = CALIB_STATUS_NON;
    ...
    memcpy(&cali_data_coef, raw_cali_data + CALI_VAL_SAVE_ADDR_OFFSET, sizeof(cali_data_coef));
    pr_info("second_light cali_data_coef = %d\n", cali_data_coef);
    if (cali_data_coef < 0)
        status = CALIB_STATUS_FAIL;
    else
        status = CALIB_STATUS_PASS;
...
static int first_light_calibration_data_send(struct shub_data *sensor)
{
    ...
    memcpy(&firstlight_calidata, firstlight_cali_data + CALI_VAL_SAVE_ADDR_OFFSET, sizeof(firstlight_calidata));
    pr_err("first_light_calibration_data_send, firstlight_calidata = %d\n", firstlight_calidata);

    err = shub_send_command(sensor, SENSOR_TYPE_FIRST_LIGHT,
                SHUB_SET_CALIBRATION_DATA_SUBTYPE,
                firstlight_cali_data, CALIBRATION_DATA_LENGTH);
    if (err < 0) {
        pr_err("Write first Light Sensor CalibratorData Fail\n");
        return err;
    }
...
static int second_light_calibration_data_send(struct shub_data *sensor)
{
    ...
    memcpy(&secondlight_calidata, secondlight_cali_data + CALI_VAL_SAVE_ADDR_OFFSET, sizeof(secondlight_calidata));
    pr_err("second_light_calibration_data_send, secondlight_calidata = %d\n", secondlight_calidata);

    err = shub_send_command(sensor, SENSOR_TYPE_SECOND_LIGHT,
                SHUB_SET_CALIBRATION_DATA_SUBTYPE,
                secondlight_cali_data, CALIBRATION_DATA_LENGTH);
    if (err < 0) {
        pr_err("Write second Light Sensor CalibratorData Fail\n");
        return err;
    }
...
static int shub_save_dual_als_cali_data(struct shub_data *sensor,
    u8 *data, u32 len, u8 id)
{
    int err;
    struct file *pfile;
    char file_path[CALIB_PATH_MAX_LENG];

    if(SENSOR_TYPE_FIRST_LIGHT == id){
        if ((strlen(CALIBRATION_NODE) + strlen(calibration_filename[10])) >
            (sizeof(file_path) - 1)) {
            dev_err(&sensor->sensor_pdev->dev,
                "calibration node path is oversize.\n");
        ...
        snprintf(file_path, sizeof(file_path), "%s%s",
            CALIBRATION_NODE, calibration_filename[10]);
    }else if(SENSOR_TYPE_SECOND_LIGHT == id){
        if ((strlen(CALIBRATION_NODE) + strlen(calibration_filename[11])) >
            (sizeof(file_path) - 1)) {
            dev_err(&sensor->sensor_pdev->dev,
                "calibration node path is oversize.\n");
...
static int set_dual_als_calib_cmd(struct shub_data *sensor, u8 id)
{
    int err;
    char data[30] = {0};
    int cali_data_dual;
    ...
    err = shub_sipc_read(sensor,
                SHUB_GET_CALIBRATION_DATA_SUBTYPE, data, CALIBRATION_DATA_LENGTH);
    if (err < 0) {
        pr_err("read RegMapR_GetLightRawData failed!\n");
        return err;
    }

    memcpy(&cali_data_dual, data + CALI_VAL_SAVE_ADDR_OFFSET, sizeof(cali_data_dual));
    pr_err("set_dual_als_calib_cmd, cali_data_dual= %d\n", cali_data_dual);

    err = shub_save_dual_als_cali_data(sensor, data, sizeof(data), id);
    if (err < 0) {
        pr_err("Save dual Light Sensor CalibratorData Fail\n");
        return err;
    }
    ...
    if (err < 0) {
        pr_err("Write %d Light Sensor CalibratorData Fail\n", sensor->cal_id);
        return err;
    }
    pr_err("set_dual_als_calib_cmd end\n");

    return 0;
}

static ssize_t dual_light_sensor_calibrator_show(struct device *dev,
                   struct device_attribute *attr, char *buf)
{
    struct shub_data *sensor = dev_get_drvdata(dev);
...
static ssize_t dual_light_sensor_calibrator_store(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    struct shub_data *sensor = dev_get_drvdata(dev);
    ...
    if (sensor->cal_id != SENSOR_TYPE_FIRST_LIGHT &&
        sensor->cal_id != SENSOR_TYPE_SECOND_LIGHT) {
        pr_err("light sensor cali cmd error\n");
        return -EINVAL;
    }

    err = set_dual_als_calib_cmd(sensor, sensor->cal_id);
    if (err < 0)
        pr_err("light sensor cali Fail!\n");

    return err < 0 ? err : count;
}
static DEVICE_ATTR_RW(dual_light_sensor_calibrator);
/*Tab A8 code for SR-AX6300-01-65 by xiongxiaoliang at 2021/08/25 end*/
...
static struct attribute *sensorhub_attrs[] = {
	...
	&dev_attr_acc_calibration_data.attr,
	/*Tab A8 code for AX6300DEV-48 by xiongxiaoliang at 2021/08/19 start*/
	&dev_attr_gyro_calibration_data.attr,
	/*Tab A8 code for AX6300DEV-48 by xiongxiaoliang at 2021/08/19 end*/
	/*Tab A8 code for SR-AX6300-01-65 by xiongxiaoliang at 2021/08/25 start*/
	&dev_attr_dual_light_sensor_calibrator.attr,
	&dev_attr_first_light_calibration_data.attr,
	&dev_attr_second_light_calibration_data.attr,
	&dev_attr_first_light_calibration_status.attr,
	&dev_attr_second_light_calibration_status.attr,
	/*Tab A8 code for SR-AX6300-01-65 by xiongxiaoliang at 2021/08/25 end*/
	NULL,
};
