...
static int power_enable(struct sprd_audio_power_info *info)
{
	...
	case SPRD_AUDIO_POWER_MICBIAS1:
		...
		value = MIC1_BIAS_EN;
		ret = sprd_power_u_bits(info, reg, value, value);
		/* HS03 code for SR-SL6215-01-201 By yuyongxiang at 20210803 start */
		if (ret < 0) {
			sp_asoc_pr_info("audio power %d enable error:%d,\n",
				info->desc.id, ret);
			return ret;
		}
		reg = sprd_power_u_bits(info, ANA_PMU5, MIC1_BIAS_V(0xf), MIC1_BIAS_V(0x5));
		if (ret < 0) {
			sp_asoc_pr_info("audio power %d enable error:%d,\n",
				info->desc.id, ret);
			return ret;
		}
		/* HS03 code for SR-SL6215-01-201 By yuyongxiang at 20210803 end */
		break;
	case SPRD_AUDIO_POWER_MICBIAS2:
		reg = ANA_PMU0;
		value = BIAS_EN;
		...
		reg = sprd_power_u_bits(info, ANA_PMU5, MIC2_BIAS_V(0xf), MIC2_BIAS_V(0x5));
		if (ret < 0) {
			sp_asoc_pr_info("audio power %d enable error:%d,\n",
				info->desc.id, ret);
			return ret;
	...
	case SPRD_AUDIO_POWER_HEADMICBIAS:
		...
		ret = sprd_power_u_bits(info, reg, value, value);
		/* HS03 code for SR-SL6215-01-200 By yuyongxiang at 20210804 start */
		if (ret < 0) {
			sp_asoc_pr_info("audio power %d enable error:%d,\n",
				info->desc.id, ret);
			return ret;
		}
		reg = sprd_power_u_bits(info, ANA_PMU5, HMIC_BIAS_V(0xf), HMIC_BIAS_V(0x1));
		if (ret < 0) {
			sp_asoc_pr_info("audio power %d enable error:%d,\n",
				info->desc.id, ret);
			return ret;
		}
		/* HS03 code for SR-SL6215-01-200 By yuyongxiang at 20210804 end */
		break;
	default:
		WARN_ON(1);
	}
